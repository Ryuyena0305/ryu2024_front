...
//만약 입력받은 데이터의 (800420-1234567) 성별의 자릿수 : 8번째 자리
// - 문자열은 " " , ' ' , ` `으로 감싼 자료
// - 문자를 여러개 저장하면 문자배열 ---> 문자열, c언어는 문자열을 배열로 만들기
// - 문자 = [ 'a', 'b', 'c' ], 문자 - ['유' , '재', '석']
// - JS는 문자열을 배열로 취급하기 때문에 배열 함수를 사용할 수 있다. [ 인덱스 사용 가능하다. ]
...
/*
    ...
    [ 2. 계산식 패턴 ]
        ...
        2. p1 == (p2+1)%3 나머지값이 p1과 같으면 p1 승리
            (p1 ==0 &&p2==3)||(p1 ==1 &&p2==1)||(p1 ==2 &&p2==2)
...
//     빈좌석 : 'X'  사용중인좌석 : 'O'
let sheetList = [ 'O' , 'X' , 'O' ];
let 예약좌석번호 = Number(prompt('[10] 좌석1, 좌석2, 좌석3 :'));
//만약에 배열내 입력받은 인덱스(좌석번호)에 'O'(대문자O)이면
if(sheetList[예약좌석번호 -1]=='O'){//-1을 하는 이유는 배열길이는 1-3 인덱스는 0-2
    console.log('예약불가');
}else{
    console.log('예약성공');
    sheetList[예약좌석번호-1] ='O'; //예약성공시 입력받은 좌석의 값 변경
}
...
let 차량번호 = prompt('[11] 차량번호 : ');
let 찾은인덱스 = carArray.indexOf(차량번호); // indexOf(찾을 데이터) : 존재하면 인덱스 반환, 없으면 -1

if(찾은인덱스 == -1){//-1이면 검색된 차량번호가 없다.
    console.log('입력한 차량번호는 주차내역이 없습니다.');
