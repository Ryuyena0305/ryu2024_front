...
//[지문9] 1차점수와 2차점수 prompt함수로 각 입력받아서 총점이 150점이상이면
//  총점이 150점이상이면 '합격' 아니면 '불합격'
//  HTML <h3>에 표시하시오.
let pride1 = Number(prompt('1차 점수 입력'));
let pride2 = Number(prompt('2차 점수 입력'));
let sum = pride1 + pride2;
let prize = sum >= 150 ? '합격' : '불합격'
document.write(`<h3>${prize}</h3>`)


//[지문9] 두사람의 이름을 prompt함수로 각 입력받아서 
//  만일 이름이 '유재석'이면  뒤에 (방장) 이라고 이름 위에 붙이고 아니면 생략한다.
//  HTML <ol>에 표시하시오.
let name1 = prompt('이름1 입력');
let name2 = prompt('이름2 입력');
name1 = name1 =='유재석'? `${name1}(방장)` : name1
name2 = name2 =='유재석'? `${name2}(방장)` : name2
// name1 변수값이 '유재석'이면 name1값 뒤에 (방장)붙이고 아니면 그대로 name1 변수 값을 수정한다.
document.write(`<ol><li>${name1} </li><li>${name2}</ol>`)